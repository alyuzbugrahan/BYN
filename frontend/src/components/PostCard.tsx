...
const PostCard: React.FC<PostCardProps> = ({ post, onUpdate, onDelete }) => {
  ...
  const renderPostContent = () => {
      ...
      return (
        <div className="space-y-3">
          <textarea
            ...
            className="w-full p-3 border border-gray-300 rounded-lg resize-none focus:ring-byn-500 focus:border-byn-500"
            rows={4}
          ...
          <div className="flex items-center justify-between">
            <select
              ...
              className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-byn-500 focus:border-byn-500"
            >
            ...
            <div className="flex space-x-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-byn-500 text-white rounded-lg hover:bg-byn-600 disabled:opacity-50"
              >
    ...
    // Basic hashtag and mention rendering
    const processedContent = content.replace(/#(\w+)/g, '<span class="text-byn-600 font-medium">#$1</span>')
                                    .replace(/@(\w+)/g, '<span class="text-byn-600 font-medium">@$1</span>');
  ...
  return (
    <div className="bg-white rounded-lg shadow-card border border-gray-200 mb-4 transition-all duration-200 hover:shadow-card-hover animate-fade-in">
      {/* Post Header */}
      <div className="p-4 border-b border-gray-100">
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-3">
            <Link to={`/profile/${post.author.id}`} className="group">
            ...
            <div>
              <Link 
                ...
                className="font-semibold text-gray-900 hover:text-byn-600 transition-colors duration-200"
              >
              ...
              <p className="text-xs text-gray-500">
                ...
                {post.visibility !== 'public' && (
                  <span className="ml-2 text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded animate-slide-up">
          ...
          {(canEdit || canDelete) && (
            <div className="relative" ref={dropdownRef}>
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 z-10 animate-slide-up">
                  <div className="py-1">
                    {canEdit && (
                      <button
                        onClick={handleEdit}
      ...
      <div className="p-4">
        ...
        {post.shared_job && !isEditing && (
          <div className="border border-gray-200 rounded-lg p-4 mb-3 bg-gray-50 transition-all duration-200 hover:bg-gray-100 hover:border-byn-300">
            <Link 
              to={`/jobs/${post.shared_job.id}`}
        ...
        {post.article_url && !isEditing && (
          <div className="border border-gray-200 rounded-lg p-4 mb-3 transition-all duration-200 hover:border-byn-300 hover:shadow-md">
            <a 
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {post.hashtags.map((hashtag) => (
              <Link
                ...
                className="text-byn-600 text-sm hover:underline hover:text-byn-700 transition-colors duration-200 px-2 py-1 rounded-full hover:bg-byn-50"
              >
                #{hashtag.name}
              </Link>
            ))}
        ...
        <div className="px-4 py-2 border-t border-gray-100 text-sm text-gray-600">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              {likesCount > 0 && (
                <span className="hover:text-byn-600 transition-colors duration-200 cursor-default">
              ...
              {commentsCount > 0 && (
                <button
                  ...
                  className="hover:text-byn-600 transition-colors duration-200"
                >
          ...
          <div className="flex items-center justify-between">
            <button
              ...
              className={`flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 transform hover:scale-105 ${
                isLiked
                  ? 'text-byn-600 bg-byn-50 hover:bg-byn-100'
                  : 'text-gray-600 hover:text-byn-600 hover:bg-gray-100'
              }`}
            >
              {isLiked ? (
            ...
            <button
              onClick={() => setShowComments(!showComments)}
              className="flex items-center space-x-2 px-4 py-2 rounded-lg text-sm font-medium text-gray-600 hover:text-byn-600 hover:bg-gray-100 transition-all duration-200 transform hover:scale-105"
            >
            ...
            >
              {isSaved ? (
                <BookmarkSolid className="w-5 h-5 animate-pulse" />
              ) : (
